...
class sample_generator(nn.Module):
    ...
    def forward(self, z, prev_x, y ,batch_size,pitch_range):
        ...
        h0_prev = self.dropout(lrelu(self.bn0_prev(self.h0_prev(prev_x)),0.2))   #[72, 16, 16, 1]
        h1_prev = self.dropout(lrelu(self.bn1_prev(self.h1_prev(h0_prev)),0.2))  #[72, 16, 8, 1]
        h2_prev = self.dropout(lrelu(self.bn2_prev(self.h2_prev(h1_prev)),0.2))  #[72, 16, 4, 1]
        h3_prev = self.dropout(lrelu(self.bn3_prev(self.h3_prev(h2_prev)),0.2))  #[72, 16, 2, 1])
...
class discriminator(nn.Module):
    def __init__(self,pitch_range):
        ...
        self.mlp = nn.Sequential(
            nn.Linear(1037, 756),
            nn.BatchNorm1d(756, eps=1e-05, momentum=0.9, affine=True),
            nn.ReLU(),
            nn.Linear(756, 512),
            nn.BatchNorm1d(512, eps=1e-05, momentum=0.9, affine=True),
            nn.ReLU(),
            nn.Linear(512, 256),
            nn.BatchNorm1d(256, eps=1e-05, momentum=0.9, affine=True),
            nn.ReLU(),
            nn.Linear(256, 128),
            nn.BatchNorm1d(128, eps=1e-05, momentum=0.9, affine=True),
            nn.ReLU()
        )
